...
* Fatal provider/config errors — bad API key, empty balance, out of stock,
  IP not allowed. These are raised as ``PakMailError`` / ``NextProxyError``.
...
class MailboxTimeoutError(RuntimeError):
    ...
    Deliberately NOT a subclass of ``PakMailError`` so the
...
class MailboxCancelled(RuntimeError):
    ...
    Also NOT a ``PakMailError`` so it is not reported as a
