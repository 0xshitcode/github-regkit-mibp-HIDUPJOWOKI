#!/usr/bin/env python3
"""CLI entry: automated GitHub sign-up with Camoufox + PakMail + NextProxy."""
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(
        prog="github-regkit",
        description="Auto-register GitHub accounts (Camoufox + PakMail + NextProxy).",
    )
    ...
    if not cfg_path.is_file() and Path("config.example.json").is_file():
        ...
        print(f"[!] created {cfg_path} from config.example.json — set your NextProxy API key")
